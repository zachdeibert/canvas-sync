...
func (m *Monitor) registerListeners(t *Task, progressListener func(*Task, float32), childrenListener func(*Task, *Task, []*Task), finishListener func(*Task)) {
...
func (m *Monitor) GetHeader() *Section {
	return m.header
}

// GetFooter gets the footer
func (m *Monitor) GetFooter() *Section {
	return m.footer
}
...
func (m *Monitor) renderHeader(text [][]string) {
	if m.layout.screenSizeChanged {
		m.relayout()
		return
	}
...
func (m *Monitor) renderFooter(text [][]string) {
	if m.layout.screenSizeChanged {
		m.relayout()
		return
	}
...
func (m *Monitor) renderProgress(row int, progress int) {
	if m.layout.screenSizeChanged {
		m.relayout()
		return
	}
...
func (m *Monitor) calculateProgressBar(progress float32) int {
...
func (m *Monitor) readScreenSize() (int, int) {
...
func (m *Monitor) detectScreenSize(locking bool) {
...
func (m *Monitor) layoutTask(task *Task, layout []monitoredTask, usedTasks int) int {
	i := usedTasks
...
func (m *Monitor) cleanupIndent(tasks []monitoredTask, before int, iter int) {
